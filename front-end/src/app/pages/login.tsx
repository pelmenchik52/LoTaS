                ...
                <form onSubmit={handleRegister} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={registerRole} onValueChange={(value) => setRegisterRole(value as UserRole)}>
                      <SelectTrigger id="register-role"><SelectValue placeholder="Оберіть роль" /></SelectTrigger>
                      <SelectContent>
                      ...
                      <div className="space-y-2 border rounded-md p-3 max-h-40 overflow-y-auto">
                        {warehouses.map((warehouse) => (
                          <div key={warehouse} className="flex items-center space-x-2">
                            <Checkbox id={`reg-${warehouse}`} checked={selectedWarehouses.includes(warehouse)}
                              onCheckedChange={() => toggleWarehouse(warehouse)} />
                            <label htmlFor={`reg-${warehouse}`} className="text-sm cursor-pointer">{warehouse}</label>
                          </div>
                        ))}
                      </div>
                    </div>
